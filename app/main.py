...
async def deposit_currency(callback):
    ...
    if not address:
        await callback.message.answer(
            f"⚠️ لم يتم إعداد طريقة {asset} بعد.\n"
            "تواصل مع الإدارة."
        )
        ...
        await callback.message.answer(
            ...
            "💳 طريقة الدفع: شام كاش\n"
            f"🆔 معرّف الاستلام:\n{address}\n\n"
            "بعد التحويل، احتفظ بإثبات العملية وأرسل تفاصيلها "
            "للإدارة للمراجعة والتأكيد."
        )
    else:
        await callback.message.answer(
            f"➕ إيداع {asset}\n\n"
            f"🌐 الشبكة: {network}\n"
            f"📍 العنوان:\n{address}\n\n"
            "بعد التحويل، أرسل رقم المعاملة (TX Hash) "
            "إلى الإدارة لإتمام المراجعة."
        )
...
async def withdraw_command(message: Message):
    ...
    await message.answer(
        f"✅ تم إنشاء طلب السحب رقم #{rid}.\n"
        "سيتم مراجعته قبل التنفيذ."
    )
